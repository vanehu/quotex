...
#ifndef QUOTER_SGT_QUOTER_SGT_P_H
#define QUOTER_SGT_QUOTER_SGT_P_H

#include <network/asio_client.h>
#include <network/sock_client.h>

#include "struct_sgt.h"
...
class QuoterSGT_P : public basicx::NetServer_X
{
public:
	QuoterSGT_P();
	~QuoterSGT_P();
...
public:
	...
	void MS_AddData_SnapshotStock( SnapshotStock_SGT& snapshot_stock_temp );
	...
	int32_t QuoteLogin_Asio();
	void QuoteLogout_Asio();
	void SendHeartCheck_Asio();
	void HandleMarketData_Asio();

	int32_t QuoteLogin_Sock();
	void QuoteLogout_Sock();
	void SendHeartCheck_Sock();
	void HandleMarketData_Sock();

	void QuotePhrase_Logon( char* mdgw_data_cache );
	void QuotePhrase_Logout( char* mdgw_data_cache );
	void QuotePhrase_SnapshotStock_SGT( char* mdgw_data_cache, size_t body_length );
...
public:
	...
	QuoteCache<SnapshotStock_SGT> m_cache_snapshot_stock;
...
public:
	std::string m_app_ver_id;
	std::atomic<bool> m_connect_ok;
	std::atomic<bool> m_logon_ok;
	std::atomic<bool> m_logout_ok;
	basicx::AsioClient m_asio_client;
	basicx::ConnectAsio* m_connect_quote;
	basicx::SockClient m_sock_client;
	thread_ptr m_thread_handle_market_data;
	char* m_mdgw_data_cache;
	size_t m_mdgw_data_cache_size;
};

#endif // QUOTER_SGT_QUOTER_SGT_P_H
